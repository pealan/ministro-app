...
In order to run this project as it is, you will need to replace the `ministro_config.json` values with the correct values from you Twitter API account. More specifically, the `Consumer Key`, `Consumer Secret`, `Access Token` and `Access Token Secret`.

## Project Structure

This project contains source code and supporting files for a serverless application that you can deploy with the SAM CLI. It includes the following files and folders.

- ministro_tweet - Code for the application's Lambda function.
...
```bash
ministro-app$ sam build --use-container
```

The SAM CLI installs dependencies defined in `ministro_tweet/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
ministro-app$ sam local invoke MinistroFunction --event events/event.json
```
...
```bash
ministro-app$ sam logs -n MinistroFunction --stack-name ministro-app --tail
```
...
```bash
ministro-app$ pip install pytest pytest-mock --user
ministro-app$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name ministro-app
```
